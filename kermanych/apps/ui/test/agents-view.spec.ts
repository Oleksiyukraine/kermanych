...
test('every kind has a Ukrainian label, and they differ', () => {
  expect(agentKindLabel('session')).toBe('власна сесія');
  expect(agentKindLabel('procedure')).toBe('процедура');
  expect(agentKindLabel('automation')).toBe('без ШІ');
  const labels = (['session', 'procedure', 'automation'] as const).map((k) => agentKindLabel(k));
