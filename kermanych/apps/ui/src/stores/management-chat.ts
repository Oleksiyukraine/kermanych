...
//      Release Notes branch calls `useReleaseNotes().create(workspaceId, …)`. The api
//      deliberately holds no write path of its own and no cloud credentials for either table.
//
// Two sections say `read_write` in @kermanych/core, so four actions reach something:
// `risk.create` / `risk.update` write a row, and `release.notes` runs a pipeline — the local
// api writes the document from git history (it is the only party that can: the commits and
// omp are on THIS machine), and this store stores it. Every other section can only be
// answered with `unsupported`, and its refusal quotes the section table.
...
export const useManagementChat = defineStore('management-chat', () => {
  ...
  // JWT. Both halves are here because a note generated but not stored costs a model turn for
  // a document nobody can find, and because this is the second way into that pipeline: the
  // form on ManagementReleasesPage is the first, and it runs exactly these two calls.
  async function generateReleaseNotes(workspaceId: string, action: ManagementReleaseNotes): Promise<void> {
    ...
    // Said BEFORE the await, and this is the reason the line exists at all: the model has
    // already stopped talking, the generation legitimately runs for tens of seconds, and
    // without a line naming what is in flight the chat is indistinguishable from a hang.
    result(
      workspaceId,
      'info',
      `Генерую реліз-ноти: ${project.name} · ${action.branch} · ${action.rangeFrom} — ${action.rangeTo}. Це може тривати до хвилини.`,
    );
    try {
      const reply = await api.generateReleaseNotes({
        projectId: project.id,
        // What only the cloud knows travels from here; the api resolves the id against its
        // own registry for the path and the project's name. Paths never leave a client.
        workspaceName: projects.workspaceById.get(workspaceId)?.name ?? '',
        branch: action.branch,
        rangeFrom: action.rangeFrom,
        rangeTo: action.rangeTo,
      });
      const created = await releaseNotes.create(workspaceId, {
        projectId: project.id,
      ...
      // The commit count is not decoration: a note written from three commits reads very
      // differently from one written from ninety, and it is the operator's first clue that a
      // range or a branch was not the one they meant.
      result(
        workspaceId,
        'info',
        `Реліз-ноти «${created.title}» готові з ${reply.commitCount} комітів і збережені у воркспейсі — дивіться розділ Release Notes.`,
      );
    } catch (e) {
      // Verbatim: «проєкт не привʼязаний на цій машині», «гілки «x» немає в локальному
      // репозиторії» and «немає комітів за період» are three different problems with three
      // different fixes, and the api already words each of them for the operator.
      result(workspaceId, 'error', `Не вдалося згенерувати реліз-ноти: ${errorText(e)}`);
    }
  }
  ...
  async function run(workspaceId: string, action: ManagementAction): Promise<void> {
    ...
    if (action.kind === 'release.notes') {
      await generateReleaseNotes(workspaceId, action);
      return;
    }
