...
describe("registry discussion branches", () => {
  it("defaults kind to 'agent' and parentSessionId to undefined", () => {
    const r = reg();
    const g = r.upsertProject({ id: "p-x", name: "g", localRepoPath: "/tmp/x" });
    const s = r.createSession({ projectId: g.id, name: "a", task: "t", worktreePath: "", branch: "b" });
  ...
  it("persists kind='discussion' and parentSessionId", () => {
    const r = reg();
    const g = r.upsertProject({ id: "p-x", name: "g", localRepoPath: "/tmp/x" });
    const parent = r.createSession({ projectId: g.id, name: "AAA", task: "t", worktreePath: "", branch: "b" });
    const child = r.createSession({
      projectId: g.id, name: "branch: AAA", task: "", worktreePath: "", branch: "",
