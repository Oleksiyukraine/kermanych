...
export function stubSkills(): SkillsService {
  return {
    ...
    // Same stance for assignments and for the per-project instruction: no project has
    // either, so every instruction is rendered from the registry default exactly as these
    // specs assert it. Without these the four instruction sites would take their catch and
    // print a warning on every launch.
